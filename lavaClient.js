...
// Двадцать секунд, а не десять.
//
// Дважды в логе: "createInvoice failed: timeout of 10000ms exceeded" — человек
// нажал "купить" и вместо ссылки на оплату увидел "попробуйте позже". Шлюз в те
// же дни отдавал 504, то есть он был жив, но медленный.
//
// Именно таймаут, а не повтор: повтор после таймаута может задвоить счёт, и это
// решение здесь принято сознательно (см. getWithRetry ниже). Ответ Discord уже
// отложен через deferReply, так что лишние секунды ожидания пользователю ничем
// не грозят — окно на ответ пятнадцать минут.
